"""BatchDeleteRelationshipsUseCase のテスト。"""
...
async def test_deletes_and_commits(deps: dict[str, AsyncMock]) -> None:
    ...
    deps["relationship_repo"].delete_many.return_value = ok_result

    uc = BatchDeleteRelationshipsUseCase(**deps)
    result = await uc.execute(relationship_ids=ids)
...
async def test_rollbacks_on_failure(deps: dict[str, AsyncMock]) -> None:
    ...
    deps["relationship_repo"].delete_many.return_value = fail_result
